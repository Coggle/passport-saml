...
class SAML {
    ...
    initialize(options) {
        ...
        if (!options.cacheProvider) {
            options.cacheProvider = new inmemory_cache_provider_1.CacheProvider({
    ...
    async generateAuthorizeRequestAsync(req, isPassive, isHttpPostBinding) {
    ...
    async generateLogoutRequest(req) {
    ...
    async requestToUrlAsync(request, response, operation, additionalParameters) {
        let buffer;
    ...
    async getAuthorizeUrlAsync(req, options) {
    ...
    async getAuthorizeFormAsync(req) {
    ...
    async getLogoutUrlAsync(req, options) {
    ...
    async getLogoutResponseUrlAsync(req, options) {
    ...
    async validatePostResponseAsync(container) {
        ...
        try {
            ...
            if (encryptedAssertions.length == 1) {
                ...
                const decryptedXml = await util.promisify(xmlenc.decrypt).bind(xmlenc)(encryptedAssertionXml, xmlencOptions);
    ...
    async validateRedirectAsync(container, originalQuery) {
        ...
        const parser = new xml2js.Parser(parserConfig);
        const doc = await parser.parseStringPromise(inflated);
    ...
    async getNameIDAsync(self, doc) {
        ...
        if (encryptedIds.length === 1) {
            ...
            const decryptedXml = await util.promisify(xmlenc.decrypt).bind(xmlenc)(encryptedDataXml, xmlencOptions);
            const decryptedDoc = new xmldom.DOMParser().parseFromString(decryptedXml);
